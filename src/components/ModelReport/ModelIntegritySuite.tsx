...
const CustomButton: React.FC<CustomButtonProps> = ({ text, disabled, modelName }) => {
  return (
    <Box display="flex" flexDirection="column" alignItems="flex-start" width="100%">
      <NavLink
        to={ {
          pathname: `/BeSLighthouse/model_integrity_suite/:${text}/:${modelName}`,
          search: ""
        } }
...
const ModelIntegritySuite: React.FC<{ name: string }> = ({ name }) => {
  ...
  const tests = [
    { key: "DataLeakageDetector", label: "DataLeakageDetector" },
    { key: "PerformanceBiasDetector", label: "PerformanceBiasDetector" },
    { key: "EthicalBiasDetector", label: "EthicalBiasDetector" },
    { key: "TextPerturbationDetector", label: "TextPerturbationDetector" },
    { key: "StochasticityDetector", label: "StochasticityDetector" },
    { key: "SpuriousCorrelationDetector", label: "SpuriousCorrelationDetector" },
    { key: "OverconfidenceDetector", label: "OverconfidenceDetector" },
    { key: "UnderconfidenceDetector", label: "UnderconfidenceDetector" }
  ];
  ...
  return (
    <Card sx={ { p: 3, minHeight: '422px', backgroundColor: '#F3F9FB', display: 'flex' } }>
      <Box display="flex" flexDirection="column" justifyContent="center" alignItems="flex-start" height="50%" textAlign="left">
        <Typography variant="h6" sx={ { fontWeight: 'bold' } }>Model Intrgrity Suite</Typography>
      </Box>
